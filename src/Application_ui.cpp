    ...
    if (m_ViewMode == ViewMode::SolarSystem) {
        ...
        if (!io.WantCaptureMouse) {
            ...
            if (auto hit = castRay(pos.x, pos.y)) {
                m_HoverLat = hit->x;
                m_HoverLon = hit->y;
                if (m_ShowPoliticalMap && m_PolMap.hasGrid())
                    m_HoverCellId = m_PolMap.findCellNearest(
                        latLonToWorld(m_HoverLat, m_HoverLon));
            }
        }
        m_PolMap.setHoverCell(m_PoliticalPaintMode ? m_HoverCellId : -1);
...
void Application::renderPanels() {
    ...
    } else if (m_SelectedCellId >= 0 && m_ShowPoliticalMap && m_PolMap.hasGrid() &&
